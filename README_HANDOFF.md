# FR Study OS Local Handoff

This bundle is a local-only handoff for reviewing and improving the FR Ledger / FR Study OS PWA with Claude Code.
...
- `app/`: Current FR Study OS PWA, built from the FR Ledger app.
...
The app is a Vite React PWA that opens into a Study OS shell styled after the polished Ind AS 23 dashboard (dark-teal rail, gold active nav, warm paper grid). Navigation: Home, Library, Chapter, Ledger, Analytics, Errors, Revision, Mocks, Vault, and a dedicated AI Tutor page.
...
The service worker registers only in production builds; dev mode actively unregisters workers and clears `fr-study-os*` caches, so local development never serves stale assets.
